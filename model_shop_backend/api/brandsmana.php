...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['id'])) {
        $id = intval($_GET['id']);
        try {
            $stmt = $conn->prepare("SELECT brand_id, name, description FROM brands WHERE brand_id = ?");
            $stmt->execute([$id]);
            $brand = $stmt->fetch(PDO::FETCH_ASSOC);
            if ($brand) {
                echo json_encode(['status' => 'success', 'data' => [$brand]]);
            } else {
            ...
            http_response_code(500);
            echo json_encode(['status' => 'error', 'message' => 'Failed to fetch brand: ' . $e->getMessage()]);
        }
    } else {
        $search = isset($_GET['search']) ? '%' . $_GET['search'] . '%' : '%';
        try {
            $stmt = $conn->prepare("SELECT brand_id, name, description FROM brands WHERE name LIKE ?");
            $stmt->execute([$search]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        echo json_encode(['status' => 'success', 'message' => 'Brand added', 'brand_id' => $conn->lastInsertId()]);
    } catch (PDOException $e) {
        http_response_code(500);
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
    ...
    try {
        $stmt = $conn->prepare("SELECT brand_id FROM brands WHERE name = ? AND brand_id != ?");
        ...
        $stmt = $conn->prepare("UPDATE brands SET name = ?, description = ? WHERE brand_id = ?");
