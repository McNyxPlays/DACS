    ...
    if (isset($_GET['conversation_id'])) {
        ...
        try {
            $stmt = $conn->prepare("
                ...
                LIMIT 1
            ");
            $stmt->execute([$user_id, $user_id, $conversation_id, $conversation_id, $user_id]);
            ...
            ");
            $stmt_update->execute([$user_id, $other_user_id]);
            ...
            $stmt = $conn->prepare("
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM messages m2 
                     WHERE m2.receiver_id = ? AND m2.sender_id = c.other_user_id AND m2.is_read = FALSE) AS unread_count
                FROM (
                    SELECT 
                        CASE 
                            WHEN sender_id = ? THEN receiver_id 
                ...
                JOIN messages m ON (m.sender_id = ? OR m.receiver_id = ?) AND m.sent_at = c.latest_sent_at
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt->execute([$sender_id, $conversation_id, $content]);
        ...
        if (isset($_FILES['media']) && $_FILES['media']['error'] === UPLOAD_ERR_OK) {
            ...
            $stmt = $conn->prepare("INSERT INTO messages_imagevid (message_id, media_url, media_type) VALUES (?, ?, ?)");
        ...
        $stmt_notify->execute([$conversation_id, $notification_message, date('Y-m-d H:i:s')]);

        $conn->commit();
        echo json_encode(['success' => true, 'message' => 'Message sent', 'message' => $message]);
    } catch (Exception $e) {
        $conn->rollBack();
        http_response_code(500);
