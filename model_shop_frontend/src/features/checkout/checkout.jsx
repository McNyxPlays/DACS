import React, { useState, useEffect } from "react";
   ...
   export default function Checkout() {
     ...
     const fetchCsrfToken = async () => {
       try {
         const response = await api.get("/orders.php?action=csrf_token", { withCredentials: true });
     ...
     const fetchCartItems = async () => {
       ...
       try {
         ...
         const response = await api.get(endpoint, { params, withCredentials: true });
         if (response.data.status === "success") {
           ...
           setError(`Failed to fetch cart: ${response.data.message || "Unknown error"}`);
           Toastify.error("Failed to fetch cart: " + (response.data.message || "Unknown error"));
         }
       } catch (err) {
     ...
     const fetchStores = async () => {
       try {
         const response = await api.get("/orders.php?action=stores", { withCredentials: true });
         if (response.data.status === "success") {
           setStores(response.data.data || []);
         } else {
           setError(`Failed to fetch stores: ${response.data.message || "Unknown error"}`);
           Toastify.error("Failed to fetch stores: " + (response.data.message || "Unknown error"));
         }
       } catch (err) {
         const errorMsg = err.response?.data?.message || err.message || "Network or server error";
     ...
     const handleApplyPromo = async () => {
       if (!csrfToken) {
         ...
         Toastify.error("CSRF token not available.");
         return;
       }
       try {
         ...
         if (response.data.status === "success") {
           ...
           setError(`Invalid promo code: ${response.data.message || "Unknown error"}`);
           setDiscount(0);
           setPromotionId(null);
     ...
     const handlePlaceOrder = async (e) => {
       ...
       try {
         const orderData = {
           ...
           promotions: promotionId ? [{ code: promoCode, applied_discount: discount / exchangeRate }] : [],
         };
         ...
         if (response.data.status === "success") {
           ...
           const payload = user
             ...
             : { session_key: sessionKey };
           try {
             const deleteResponse = await api.delete(endpoint, { data: payload, withCredentials: true });
